...
module.exports = function(options) {
  ...
  app.post('/auth/api/v1/mock', function(req, res) {
    ...
    switch (req.accepts(['json'])) {
      case 'json':
        urlutil.isLogin(req.body.name, req.body.password, function (loginSuccess, emailDB) {
          ...
          .then(function() {
            if(loginSuccess){
              log.info('"name:%s, email:%s login in successed"', req.body.name, emailDB)
              ...
              log.info('"%s failed to login in"', req.body.name)
              res.status(403)
              .json({
                success: false
